...
import { ingestLead, orgHasActiveAccess } from "@/lib/automation/ingest-lead.server";
      ...
      POST: async ({ request }) => {
        ...
        try {
          await ingestLead({
            ...
            ownerUserId: HARDCODED_USER_ID,
            source: (leadSource as "n8n") ?? "n8n",
            fullName,
            email,
            phone,
            propertyInterest,
            message,
            status: d.status ?? "New",
            aiReply: d.ai_reply ?? null,
            receivedAt: d.created_at ?? null,
          });
        } catch (e) {
